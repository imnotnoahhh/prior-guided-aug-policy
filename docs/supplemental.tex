...
We provide an additional shot scaling study to characterize how augmentation behaviors change as data availability varies. This study uses repeated class-wise subsampling trials (subsets may overlap across trials), which is different from the disjoint StratifiedKFold protocol used in the main experiments. Therefore, we use this study to analyze scaling trends and failure modes rather than to replace the main results.
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Shot scaling study.} Different from the main experiments (disjoint StratifiedKFold), this sweep uses repeated subsampling trials and is intended to study scaling trends and failure modes, not to replace the main protocol. Note that a smaller std may coincide with consistently low accuracy in extreme low-shot settings, reflecting optimization collapse rather than robustness.}
