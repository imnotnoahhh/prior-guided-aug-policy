...
We consider a search space of $K=8$ candidate operations (e.g., ColorJitter, RandomErasing), each parameterized by Magnitude ($m$) and Probability ($p$). Following standard practice, we treat the optimal augmentation configuration $\theta^*$ as a point in a continuous 2D manifold, allowing precise tuning.
...
\textbf{Phase A: Screening.} We started with a quick scan using Sobol sequences. This let us quickly drop unstable operations (like high-magnitude geometric distortions) before wasting a lot of computing power.
